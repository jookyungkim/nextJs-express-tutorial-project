...
function uploadImagesAPI(data) {
  return axios.post("/post/images", data); // formData 그대로 넣어줘야 한다.
}
...
function loadUserPostsAPI(data, lastId) {
  return axios.get(`/user/${data}/posts?lastId=${lastId || 0}`); // lastId undefind 인경우 0으로 치환
}
...
/* 
   request 가 일어나면 10초간 잠시 기다렸다가 실행된다.
...
function* watchLoadUserPosts() {
  yield throttle(10000, LOAD_USER_POSTS_REQUEST, loadUserPosts);
}
